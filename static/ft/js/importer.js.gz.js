...
Importer.prototype.initialize = function () {
    ...
    this.$el.find(".hints > .fa-question-circle").hover(function () {
        $(this).next(".tooltip").fadeIn()
    }, function () {
        $(this).next(".tooltip").fadeOut()
    });
    if (a.studio) {
        a.studio.bind("userAssetReady", $.proxy(this.onAssetReady, this)).bind("userAssetDelete", $.proxy(this.onAssetDelete, this)).bind("imageFrameSelect", $.proxy(this.onImageFrameSelect, this)).bind("imageFrameDeselect", $.proxy(this.onImageFrameDeselect, this))
    }
    $(".ga-importer")[0].addEventListener('paste', async (e) => {
        ...
        a.addFiles(e.clipboardData.files)
    })
};
...
Importer.prototype.uploadFile = function (c) {
    ...
    c.status("uploading");
    var b = new FormData();
    b.append("publish", c.publish)
    b.append("file", c.file);
    b.append("subtype", c.subtype);
    c.xhr = $.ajax({
        type: "POST",
        url: "https://flashthemes.net/ajax/saveAsset",
        data: b,
        xhr: function () {
...
ImporterFile.prototype.initialize = function () {
    ...
    if (a == "image") {
        ...
        if (a == "flash") {
            ...
            if (a == "audio") {
                ...
                this.setMenu("sound");
                this.status("waiting")
                this.$el.find("#publish").addClass("hidepublish").removeClass("showpublish")
            } else {
                this.status("pending");
                if (a == "font") {
    ...
    this.$el.on("click", '[data-action="add-to-scene"], [data-action="add-to-frame"]', function (c) {
        ...
        b.publish = b.$el.find('input[name="publish"]').is(':checked') || false;
        b.status("pending");
        b.setIcon("");
...
ImporterFile.prototype.checkVideoStatus = function () {
    var a = this;
    $.post("https://flashthemes.net/ajax/getAssetVideoStatus/" + this.asset.id, function (b) {
        if (b.suc) {
...
ImporterFile.prototype.checkFontStatus = function () {
    var a = this;
    $.post("https://flashthemes.net/ajax/getAssetFontStatus/" + this.asset.id, function (b) {
        if (b.suc) {
            if (b.status == "completed") {
                a.asset.data = b.asset_data;
                if (b.thumbnail) {
...
ImporterFile.defaults.options = {
    accept_mime: ["image/png", "image/jpeg", "image/gif", "image/bmp", "image/webp", "audio/mpeg", "audio/wav", "audio/x-wav", "audio/vnd.wave", "audio/wave", "audio/mp3", "audio/mp4", "audio/ogg", "audio/vorbis", "audio/aac", "audio/flac", "audio/m4a", "audio/x-m4a", "application/mp4", "video/mp4", "video/mpeg4", "video/x-flv", "video/x-ms-wmv", "application/x-shockwave-flash", "font/x-font-ttf", "font/vnd.ms-opentype"],
    restricted_mime: [],
